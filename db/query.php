...
function printOptionsBySql($sql,$value,$show){
	global $con;
	$result = $con->query($sql);
	echo "<option>Choose</option>";
...
function printOptionsWithCondition($table,$value,$show,$condition){
	global $con;
...
function printTable($table,$headers,$column){
	global $con;
...
function printTableRelease($sql,$headers,$column,$edit,$table,$r,$view=false){
	...
	while($res=mysqli_fetch_array($result)){
		...
		<tr>
			...
			for($i=0;$i<count($column);$i++){
				...
				if($column[$i]=='release_date'){
						if($res[$column[$i]]){
							echo "<td>".$res[$column[$i]]."</td>";
						}
						else{
							echo "<td><a class='btn btn-success' href='releasebed.php?id=".$res["id"]."'>Release Now</a></td>";
						}
				}
				else{
					echo "<td>".$res[$column[$i]]."</td>";
	...
	while($res=mysqli_fetch_array($result)){
		...
		<tr>
			...
			echo "<td>";
				...
				<?php
			?>
			<!--<a onclick='return confirm("Are you sure?")' class='btn btn-sm btn-danger' href='delete.php?id=<?=$res["id"]?>&t=<?=$table?>&r=<?=$r?>'>Delete</a>-->
		</tr>
		<?php
	}?>
	</tbody>
...
function printTableWithSchedule($sql,$headers,$column,$edit,$table,$r,$sl){
	global $con;
...
<tbody>
	...
	while($res=mysqli_fetch_array($result)){
		...
		<tr>
			...
			?>
			<td><a  class='btn btn-sm btn-warning' href='<?=$sl?>?id=<?=$res["id"]?>'>Schedule</a></td>
			<?php
			echo "<td>";
				echo "<a class='btn btn-sm btn-success' href='".$edit."?id=".$res['id']."'><i class='fas fa-edit' aria-hidden='true'></i></a>";
				?>
